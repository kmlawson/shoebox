...
//Get the recipient's email list from a list of emails in a tab delimited list
$tab=chr(9);
$item=urldecode($_POST["item"]);
$title=urldecode($_POST["title"]);
$zaemail=$_POST["zaemail"];
$zaname=$_POST["name"];
$zamessage=$_POST["Message"];

require_once('recaptchalib.php');
$privatekey = "6LdYDsASAAAAAL-SwjBxR0H70yY_aCdj_VI949YF";
...
function mail_it($content, $subject, $email, $recipient) {
   global $attachment_chunk, $zaemail, $zamessage, $item, $title, $zaname, $attachment_name, $attachment_type, $attachment_sent, $bcc;
  // $ob = "----=_OuterBoundary_000";
  // $ib = "----=_InnerBoundery_001";
   
   $headers  = "MIME-Version: 1.0\r\n"; 
   $headers .= "From: ".$zaemail."\n"; 
   //$headers .= "To: ".$recipient."\n"; 
   //$headers .= "Reply-To: ".$email."\n";
   //if ($bcc) $headers .= "Bcc: ".$bcc."\n"; 
   //$headers .= "X-Priority: 1\n"; 
   //$headers .= "X-Mailer: DT ".VERSION."\n"; 
   ...
   $message .= "You have received a suggestion or correction for the Shobox. \n\n Message From: $zaname \n Email Address: $zaemail \n Item Link: $item \n Item Title: $title \n\n Message Contents: \n\n $zamessage \n";
   //$message .= $content."\n\n";
   //$message .= "\n--".$ib."--\n";
...
// were used to Matt's.pl which used required instead.
if ($required)
   $require = $required;
...
// send it off
mail_it(stripslashes($content), ($subject)?stripslashes($subject):"Shoebox Correction or Suggestion", $zaemail, $recipient);
...
if ($redirect) {
   header("Location: http://huginn.net/shoebox/thanks.html");
   exit;
} else {
	header("Location: http://huginn.net/shoebox/thanks.html");
